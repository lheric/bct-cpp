 ...
 */
gsl_matrix* bct::motif4funct_wei(const gsl_matrix* W, gsl_matrix** Q, gsl_matrix** F) {
						...
						if (find_V3 != NULL ) {
							for (int i_find_V3 = 0; i_find_V3 < (int)find_V3->size; i_find_V3++) {
								...
								if (m > 0) {
									...
									gsl_vector* M4_cols = sequence_double(0, M4->size2 - 1);
									gsl_matrix* M = log_ord_index(M4, ind, M4_cols);
									gsl_vector_free(M4_cols);
									...
									// l=N4(ind);
									gsl_vector* l = logical_index(N4, ind);
									...
									double IQF_cols[] = { (double)u, (double)v1, (double)v2, (double)v3 };
									gsl_vector_view IQF_cols_vv = gsl_vector_view_array(IQF_cols, 4);
 ...
 */
gsl_vector* bct::motif4funct_wei_v(const gsl_matrix* W, gsl_vector** Q, gsl_vector** F) {
	gsl_matrix* _Q;
	gsl_matrix* _F;
	gsl_matrix* _I = motif4funct_wei(W, &_Q, &_F);
	if (Q != NULL) {
		*Q = sum(_Q, 2);
	}
	gsl_matrix_free(_Q);
