...
for i = 1:size(m)(2)
	...
	%this is only an approximate test condition
	bct_test(sprintf("randmio_dir_connected %s", mname{i}), all(abs(deg_randmio_dir-deg_randmio_dir_cpp) == 0)); 
end
...
for i = 1:size(m)(2)
	...
	deg_randmio_dir_cpp = degrees_dir(R_cpp);
	%this is only an approximate test condition
	bct_test(sprintf("randmio_dir %s", mname{i}), all(abs(deg_randmio_dir-deg_randmio_dir_cpp) == 0)); 
end
...
for i = 1:size(m)(2)
	...
	%this is only an approximate test condition
	bct_test(sprintf("randmio_und_connected %s", mname{i}), all(abs(deg_randmio_und-deg_randmio_und_cpp) < 10)); 
end
