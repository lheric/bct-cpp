...
gsl_matrix* makerandCIJ_wu(int, const gsl_vector*);
...
gsl_matrix* bct::makerandCIJ_wu(int N, int K, double wmin, double wmax) {
	...
	gsl_rng_free(rng);
	gsl_matrix* ret = makerandCIJ_wu(N, w);
	gsl_vector_free(w);
	return ret;
}
...
gsl_matrix* makerandCIJ_wu(int N, const gsl_vector* w) {
	using namespace bct;
