...
DEFUN_DLD(findpaths_cpp, args, , "Wrapper for C++ function.") {
	...
	if (!error_state) {
		...
		gsl_matrix* allpths;
		gsl_matrix* util;
		std::vector<gsl_matrix*> Pq = bct::findpaths(CIJ_gsl, sources_gsl, qmax, &tpath, &plq, &qstop, &allpths, &util);
		gsl_matrix_add_constant(allpths, 1.0);
		octave_value_list ret;
		ret(0) = octave_value(bct_test::from_gsl(Pq, 1));
		ret(1) = octave_value(tpath);
		ret(2) = octave_value(bct_test::from_gsl(plq, 1));
		ret(3) = octave_value(qstop);
		ret(4) = octave_value(bct_test::from_gsl(allpths));
		ret(5) = octave_value(bct_test::from_gsl(util, 0, 1));
