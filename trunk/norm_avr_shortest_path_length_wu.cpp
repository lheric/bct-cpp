...
/*
 *Computes the normalized average shortest path length for undirected & weighted network
 *
 */

double bct::norm_avr_shortest_path_length_wu(gsl_matrix* m) {
	
	if (safe_mode) check_status(m, WEIGHTED | UNDIRECTED, "norm_avr_shortest_path_length_wu");
	if (m->size1 != m->size2) {
